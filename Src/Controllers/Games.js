...
exports.rockpaper = async (req, res, next) => {
  try {
    const answerUser = req.query.answer
    ...
    const possibleAnswer = ['rock', 'paper', 'scissors']
    ...
    const answer = Math.floor(Math.random() * outcomes.length)
...
exports.eightball = async (req, res, next) => {
  try {
    ...
    const randomAnswer = ans[Math.floor(Math.random() * ans.length)]
